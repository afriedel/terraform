...
func credentialsConfigFile() (string, error) {
	...
	return filepath.Join(configDir, "credentials.tf.json"), nil
}
...
type CredentialsSource struct {
	...
	// any writes to the special credentials.tf.json file.
	configured map[svchost.Hostname]cty.Value

	// unwritable describes any credentials explicitly configured in the
	// CLI config in any file other than credentials.tf.json. We cannot update
	// these automatically because only credentials.tf.json is subject to
	...
	// of the credentials.tf.json file.
